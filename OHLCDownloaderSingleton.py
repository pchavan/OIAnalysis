...
class OHLCDownloaderSingleton:
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            cls._instance = super(OHLCDownloaderSingleton, cls).__new__(cls, *args, **kwargs)
    ...
    def _set_cookie(self):
        request = self._sess.get(url_oc, headers=headers, timeout=5)
        self._cookies = dict(request.cookies)

    def _get_data(self, url):
        self._set_cookie()
        response = self._sess.get(url, headers=headers, timeout=5, cookies=self._cookies)
        if (response.status_code == 401):
            self._set_cookie()
            response = self._sess.get(url_nf, headers=headers, timeout=5, cookies=self._cookies)
    ...
    def _set_header(self):
        ...
        global bnf_nearest
        global nf_nearest
        response_text = self._get_data(url_indices)
    ...
    def _get_date_and_x_months_ago(self, x=6):
    ...
    def _rename_index_df_columns(self, df):
    ...
    def _process_for_comma(self, df=None):
        try:
    ...
    def _convert_df_ohlc_to_float(self, df=None):
        try:
    ...
    def download_ohlc_from_nse(self, symbol=None, resample_method="W", period="12M"):
        tickers = t.Tickers()
        ...
        if index_or_stock == "INDEX":
            return self._download_index_ohlc_from_nse(symbol=dl_code, resample_method=resample_method, period=period)
        elif index_or_stock == "STOCK":
            return self._download_stock_ohlc_from_nse(symbol=dl_code, resample_method=resample_method, period=period)


    def _download_index_ohlc_from_nse(self, symbol=None, resample_method="W", period="12M"):

        global _nse_index_dl_url
        # _set_header()
        # _set_cookie()
        to_from = self._get_date_and_x_months_ago(int(period[:-1]))
        dl_url = _nse_index_dl_url.format(urllib.parse.quote(symbol), to_from[1], to_from[0])
        response = self.__get_data(dl_url)
        json_data = json.loads(response)
        json_data = json_normalize(json_data['data']['indexCloseOnlineRecords'])
        df_index_ohlc = pd.DataFrame(json_data)
        df_index_ohlc = self._rename_index_df_columns(df_index_ohlc)
    ...
    def _download_stock_ohlc_from_nse(self, symbol=None, resample_method="W", period="12M"):
        global _nse_cm_dl_url
        to_from = self._get_date_and_x_months_ago(int(period[:-1]))
        dl_url = _nse_cm_dl_url.format(urllib.parse.quote(symbol), to_from[1], to_from[0])
        response = self._get_data(dl_url)
        ...
        df_stock_ohlc = self._process_for_comma(df_stock_ohlc)
        df_stock_ohlc = self._convert_df_ohlc_to_float(df_stock_ohlc)
        return df_stock_ohlc

    def download_index_oc_from_nse(self):
        return []

    def download_stock_oc_from_nse(self):
        return []

    def download_index_ohlc_from_yahoo(self, symbol=None, interval="1wk", start=None, end=None, period="1y"):
        try:
    ...
    def download_stock_ohlc_from_yahoo(self, symbol=None, interval="1wk", start=None, end=None, period="1y"):
        return self.download_index_ohlc_from_yahoo(self, symbol=symbol, interval=interval, start=start, end=end, period=period)
